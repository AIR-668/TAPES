# BiomedBERT + Set2Sequence Training Container with WANDB + cloudflared
Bootstrap: docker
From: pytorch/pytorch:2.1.0-cuda12.1-cudnn8-runtime

%post
    apt-get update && apt-get install -y --no-install-recommends \
        python3 python3-pip git wget curl ca-certificates unzip \
        cloudflared
...
%labels
    ...
    Version="1.0"
    Description="Apptainer container for pseudotime prediction with BiomedBERT, wandb, and cloudflared"

%runscript
    echo "[Container Ready] To run: python3 train_model.py or ./run_wandb_tunnel.sh"